...
- Write the release notes in every locale the package ships; `startos/manifest/i18n.ts` lists them. The field takes any subset and its type constrains no key, so `npm run check` passes on a translation left describing the previous version and on a mistyped locale key alike. The registry resolves a locale to the exact key, then to the first key sharing its language prefix, then to the first key in the object — there is no English fallback, so a broken key serves whichever locale happens to come first. Read every entry before committing.
