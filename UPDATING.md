...
```
docker buildx imagetools inspect ghcr.io/start9labs/startos-registry:master \
...
docker run --rm --entrypoint start-registry \
  ghcr.io/start9labs/startos-registry@sha256:<digest> --version
```
...
```
docker buildx imagetools inspect ghcr.io/start9labs/startos-registry@sha256:<digest> \
...
- Set `images['startos-registry'].source.dockerTag` in `startos/manifest/index.ts` to `ghcr.io/start9labs/startos-registry@sha256:<digest>`.
