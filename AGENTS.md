...
- **The image is pinned by digest, with no `arch` declared.** Upstream publishes no per-release tag, so a bump re-resolves the digest `:master` points at — `UPDATING.md` has the procedure. The SDK defaults the image to `aarch64`, `x86_64` and `riscv64`, so the pinned index has to cover all three.
