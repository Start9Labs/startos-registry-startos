...
- **The image is pinned by digest, with no `arch` declared.** Upstream publishes no per-release tag, so a bump re-resolves the digest `:master` points at — `UPDATING.md` has the procedure. Architecture support follows the pinned index rather than the manifest.
