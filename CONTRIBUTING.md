...
This repo packages the [StartOS Registry](https://github.com/Start9Labs/start-os/tree/master/core/startos/src/registry/) for StartOS.
...
The registry source lives in the [StartOS monorepo](https://github.com/Start9Labs/start-os/tree/master/core/startos/src/registry/) and ships as the prebuilt `ghcr.io/start9labs/startos-registry` image. There is no Dockerfile in this repo; the package pulls the upstream image straight from GHCR. To track a new upstream release:

1. Bump `dockerTag` in `startos/manifest/index.ts` to `ghcr.io/start9labs/startos-registry:<new tag>`.
